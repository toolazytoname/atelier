...
*atelier*（法语：**工坊**）是一个自包含、**可丢弃的 Linux 开发沙箱**。
代码、构建、测试、依赖全部装在一个 OrbStack 的 Linux 虚拟机里；宿主
Mac 被压缩成一个终端、一个浏览器标签页，外加 OrbStack 本身。什么都
不装到你的 Mac 上——没有 Node、没有 Python、没有 Go、没有 Rust、没有
MCP server。一切都活在 VM 里，跟 VM 一起死，5 分钟就能从零重建。
...
> open-design 是一个**独立、可选的项目**。atelier 自带一份桥配置
> （`.mcp.json`）并替你起守护进程，但没有它也能用——只是丢掉了设计
> 感知那部分功能。见
> [FAQ](FAQ.md#whats-the-relationship-between-atelier-and-open-design)。

## 四大支柱

atelier 落地四个诉求。只有支柱 4（沙箱）随这个仓库一起发布；支柱
1–3 是**推荐的配套工具**——没它们 atelier 也能跑，只是丢掉那个功能。
...
| 2 | **设计对齐 Open Design** | 拿 `mcp__open-design__*` 当 spec、`lazyweb_search` 找参考、`frontend-design` / `ui-ux-pro-max` skills。 |
| 3 | **抓自测漏掉的** | `verify` skill、`e2e-runner`、多 agent `council`、Playwright 截图。 |
| 4 | **隔离 VM** | **已内置。** OrbStack Ubuntu 24.04 VM，所有工具在里面，`bin/devbox reset` 5 分钟重建，宿主不动。 |
...
Desktop 更快更轻，而且 VM 可丢弃，`bin/devbox reset` 几秒就重建。跟
...
`orb: command not found`、VM 没起、`localhost:7456` 上 `connection
refused`、VM 里看不到 token）。从可疑 VM 最快的恢复永远是
