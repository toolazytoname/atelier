...
`make setup` 已经把 `plugin/skills/atelier` 软链到 `~/.claude/skills/`，
所以这些说法会自动把命令路由到 VM——没有额外的安装步骤：
...
任何时候 `make install-skill` 重跑一次（比如拉了新 skill 改动），
幂等覆盖软链即可。
...
│   （→ [完整工具链及版本](FAQ.md#what-does-make-setup-install)）
...
├── plugin/                        # Claude Code skill（`make install-skill` 自动软链）
